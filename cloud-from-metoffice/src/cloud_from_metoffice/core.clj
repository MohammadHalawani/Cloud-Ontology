...
;; #+begin_src clojure
(ns clouds.cloud-from-metoffice
...
;; #+begin_src clojure
(defontology metoffice_cloud :iri "example.com")
;; #+end_src
...
;; and in deeper view, we divided the following two variants into
...
(deftier Form
  ...
  :functional false
  :cover false
  :comment "Used to link clouds' genus, species or variants
...
(refine bornOf
        :range Genus
        :super hasOrigin
        :subchain hasVariant bornOf)
(defoproperty changedFrom)
(refine changedFrom
        :range Genus
        :super hasOrigin
        :subchain hasVariant changedFrom)
;; #+end_src
...
;; #+begin_src clojure
(defclass CumuliformCloud  :equivalent (owl-and Cloud (form Cumuliform)))
(defclass NimbiformCloud  :equivalent (owl-and Cloud (form Nimbiform)))
(defclass AltiformCloud  :equivalent (owl-and Cloud (form Altiform)))
(defclass StratiformCloud  :equivalent (owl-and Cloud (form Stratiform)))
(defclass CirriformCloud  :equivalent (owl-and Cloud (form Cirriform)))
;; #+end_src
...
;; #developer comment:
;; cumulogenitus is created with other genitus variants
;; #+begin_comment
;; #+begin_src clojure
...
See https://en.wikipedia.org/wiki/List_of_cloud_types#Informal_terms_officially_proposed_for_WMO_classification"
           :comment "Proposed as a WMO genitus variant."
...
;; #developer comment:
;; Creating (genitus) and (mutatus)subclasses of variant from each genus
...
;; #+begin_src clojure
(doall (map genitus (subclasses Genus)))
(doall (map mutatus (subclasses Genus)))
;; #+end_src

;; #developer comment:
;; Creating clouds for each classification (Genus, Species, Variant,
     ...
     (owl-class
      ...
      :equivalent  (owl-and Cloud
                            (cond
                             ...
                             (superclass? cls Variant) (variant cls)
...
(defn clouds-for-classifications [& cls]
  (doall (map cloud-for-classification (flatten (for [x (map subclasses cls)]
                             (for [cls x]
                               cls))))))
;; #+end_src
...
(apply clouds-for-classifications (direct-subclasses Variant))
;; #+end_src
...
;; #+begin_src clojure
(defn cloud
  [{:keys [genusCloud speciesCloud variantCloudVector
  ...
  (let [genus (if genusCloud
        ...
        variantList (if variantCloudVector
                        (dofor [variant variantCloudVector]
                          (str/replace (l/resolve-entity variant) "Cloud" "")) "")
        ...
        related-terms (flatten [genus species variantList supplementaryFeatureList accessoryList])
        entity-name (str genus
                         species
                         (str/join variantList)
                         (str/join supplementaryFeatureList)
                         (str/join accessoryList) "Cloud")
        ]
    (let [clses (remove nil? [genusCloud speciesCloud variantCloudVector
                              supplementaryFeatureCloudVector accessoryCloudVector])]
    (intern-owl-string entity-name
...
https://en.wikipedia.org/wiki/List_of_cloud_types#Mutatus_mother_clouds")
                                         (str v " variant column in the following table
https://en.wikipedia.org/wiki/List_of_cloud_types#WMO_varieties")))
                     (if (seq acce)
...
(defn create-clouds [& m]
  (doall (map create-specific-clouds m)))
;; #+end_src

;; #developer comment:
;; Creating different clouds that exists in reality and combines
;; different genus, species and variants
...
;; #developer comment:
;; Creating clouds with more than a variant with their annotation
...
(doseq [opacity [TranslucidusCloud PerlucidusCloud OpacusCloud]
  ...
  (let [v (cond
    ...
    (cloud {:genusCloud gen
            ...
            :variantCloudVector  [opacity pattrn]
            :coment (str "A cloud with 2 variants (opacity-based and pattern-based)."
                         "V-" v
                         "at " link)
            :annot  [link]})))
...
(doseq [opacity [TranslucidusCloud OpacusCloud]]
  ...
  (cloud {:genusCloud AltostratusCloud
          :variantCloudVector  [opacity UndulatusCloud]
          :coment (str "A cloud with 2 variants (opacity-based and pattern-based)."
                         "V-" v
                         "at " link)
          :annot [link]})))
...
(defn addPrnc [& cls]
  (doall (map prnc (flatten (for [x (map subclasses cls)]
                             (for [cls x]
                               cls))))))
;; #+end_src
...
(apply addPrnc (direct-subclasses Variant))
;; #+end_src
